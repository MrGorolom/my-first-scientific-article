...
We employed leave-one-speaker-out (LOSO) cross-validation to ensure strict speaker independence. Feature standardization was performed within each training fold to prevent data leakage. Model selection used nested cross-validation with grid search over the specified parameter spaces.
...
Complementing our statistical significance analysis, we examined feature importance patterns using Random Forest classifiers to identify modality-specific acoustic biomarkers. Figure~\ref{fig:feature_importance_combined} presents the top discriminative features across all clinical scales, revealing systematic differences in how each speech task captures cognitive-linguistic deficits.

\begin{figure}[h!]
\centering
\includegraphics[width=0.95\textwidth]{feature_importance_combined.png}
\caption{Modality-specific feature importance patterns across clinical instruments. (A) MoCA classification shows complete divergence with spontaneous description capturing spectral dynamics and reading emphasizing harmonicity. (B) MMSE classification exhibits partial overlap in spectral features with complementary specializations. (C) CDR classification demonstrates shared core features with modality-specific variations in loudness processing.}
\label{fig:feature_importance_combined}
\end{figure}
...
These systematic biomarker differences provide mechanistic explanations for the observed performance patterns and support the strategic selection of speech tasks based on specific clinical assessment goals.

\section{Results and Discussion}

\subsection{Key Findings and Performance Patterns}

Our systematic comparison reveals several critical patterns with direct clinical implications. The superior performance of spontaneous description for MoCA ($R^2$ = 0.14 vs. -0.04) and CDR ($R^2$ = 0.15 vs. 0.04) prediction demonstrates its enhanced sensitivity to executive function and complex attention deficits. This performance advantage likely stems from spontaneous speech's engagement of multiple cognitive domains simultaneously—lexical retrieval, syntactic planning, and discourse organization—which are particularly vulnerable in early cognitive decline.

Feature importance analysis (Figure~\ref{fig:feature_importance_combined}) reinforces these quantitative findings, revealing the mechanistic basis for performance differences through distinct biomarker patterns across modalities.

The ROC AUC values for classification tasks (Table~\ref{tab:classification_results}) further reinforce this pattern. While reading tasks showed reasonable discrimination for MMSE-based classification (AUC = 0.79), they performed poorly for MoCA-based detection (AUC = 0.39), suggesting limited sensitivity to the subtle cognitive-linguistic impairments captured by this instrument.

\subsection{Biomarker Interpretation and Clinical Relevance}

The feature significance analysis (Table~\ref{tab:significant_features}) provides mechanistic insights into these performance differences. Loudness distribution parameters emerged as universal biomarkers across both modalities, with the 20th percentile of loudness achieving high significance ($p < 10^{-4}$). This may reflect reduced vocal projection or dynamic range compression due to impaired respiratory control or motivational deficits.
...
This study provides compelling evidence for task-specific advantages in speech-based dementia screening. While spontaneous description demonstrates superior diagnostic performance, particularly for executive function assessment, standardized reading offers practical advantages for large-scale implementation. The optimal approach likely involves strategic task selection based on specific clinical contexts and assessment goals.

Our findings establish that speech tasks are not interchangeable but rather provide complementary windows into cognitive-linguistic functioning. This nuanced understanding enables more targeted protocol design and moves the field toward evidence-based selection of speech tasks for specific screening scenarios.

The identified acoustic biomarkers, particularly the universal significance of loudness distribution parameters, provide concrete targets for future biomarker development and clinical translation. As digital speech assessment moves closer to clinical implementation, this work provides crucial empirical guidance for optimizing task selection and interpretation.
